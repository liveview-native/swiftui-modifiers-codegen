import SwiftUI
import SwiftSyntax

/// Generated modifier enum for ScaleEffectModifier modifiers.
///
/// This enum provides type-safe access to SwiftUI view modifiers.
/// Generated by ModifierSwift.
public enum ScaleEffectModifier: Sendable {
    case scaleEffectWithCGSizeUnitPoint(CoreFoundation.CGSize, anchor: SwiftUICore.UnitPoint)
    case scaleEffectWithCGFloatUnitPoint(CoreFoundation.CGFloat, anchor: SwiftUICore.UnitPoint)
    case scaleEffectWithCGFloatCGFloatUnitPoint(x: CoreFoundation.CGFloat, y: CoreFoundation.CGFloat, anchor: SwiftUICore.UnitPoint)
}

extension ScaleEffectModifier: RuntimeViewModifier {
    public static var baseName: String { "scaleEffect" }

    public init(syntax: FunctionCallExprSyntax) throws {
        switch syntax.arguments.count {
        case 2:
            if let value0 = CoreFoundation.CGSize(syntax: (syntax.arguments.count > 0 ? syntax.arguments[0].expression : nil)!) {
                let anchor = { if let expr = syntax.argument(named: "anchor")?.expression, let parsed = SwiftUICore.UnitPoint(syntax: expr) { return parsed } else { return .center } }()
                self = .scaleEffectWithCGSizeUnitPoint(value0, anchor: anchor)
            } else if let value0 = CoreFoundation.CGFloat(syntax: (syntax.arguments.count > 0 ? syntax.arguments[0].expression : nil)!) {
                let anchor = { if let expr = syntax.argument(named: "anchor")?.expression, let parsed = SwiftUICore.UnitPoint(syntax: expr) { return parsed } else { return .center } }()
                self = .scaleEffectWithCGFloatUnitPoint(value0, anchor: anchor)
            } else {
                throw ModifierParseError.invalidArguments(modifier: "ScaleEffectModifier", variant: "multiple variants", expectedTypes: "CoreFoundation.CGSize, SwiftUICore.UnitPoint or CoreFoundation.CGFloat, SwiftUICore.UnitPoint")
            }
        case 3:
            let x: CoreFoundation.CGFloat = if let expr = syntax.argument(named: "x")?.expression, let parsed = CoreFoundation.CGFloat(syntax: expr) { parsed } else { 1.0 }
            let y: CoreFoundation.CGFloat = if let expr = syntax.argument(named: "y")?.expression, let parsed = CoreFoundation.CGFloat(syntax: expr) { parsed } else { 1.0 }
            let anchor: SwiftUICore.UnitPoint = if let expr = syntax.argument(named: "anchor")?.expression, let parsed = SwiftUICore.UnitPoint(syntax: expr) { parsed } else { .center }
            self = .scaleEffectWithCGFloatCGFloatUnitPoint(x: x, y: y, anchor: anchor)
        default:
            throw ModifierParseError.unexpectedArgumentCount(modifier: "ScaleEffectModifier", expected: [2, 3], found: syntax.arguments.count)
        }
    }

    public func body(content: Content) -> some View {
        switch self {
        case .scaleEffectWithCGSizeUnitPoint(let value0, let anchor):
            content.scaleEffect(value0, anchor: anchor)
        case .scaleEffectWithCGFloatUnitPoint(let value0, let anchor):
            content.scaleEffect(value0, anchor: anchor)
        case .scaleEffectWithCGFloatCGFloatUnitPoint(let x, let y, let anchor):
            content.scaleEffect(x: x, y: y, anchor: anchor)
        }
    }
}